...
function MainAdminSidebar({ menuItems, activeTab, setActiveTab }) {
  ...
  return (
    <div className="relative">
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full bg-white shadow-lg shadow-gray-400 w-64 z-50 transform transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0`}
      >
        ...
        <nav className="mt-14 px-3">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full rounded-lg flex text-lg items-center gap-8 mt-4 px-10 py-2 text-right ${activeTab === item.id
